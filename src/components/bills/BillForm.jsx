...
const BillForm = ({ initialData = null, onClose }) => {
    ...
    return (
        <div className="space-y-6 bg-black-900 p-6 rounded-lg shadow-lg">
            <div>
                <h2 className="text-2xl font-semibold text-slate-200">
                    {initialData ? 'Edit Bill' : 'Add New Bill'}
                </h2>
                <p className="mt-1 text-slate-400">
                    {initialData ? 'Update the bill details below.' : 'Enter the bill details below.'}
                </p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6 bg-orange-200 p-6 rounded-lg shadow-lg">
                {/* Description Field */}
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className={`input-control pl-10 text-white${errors.description ? 'border-red' : 'border-slate-700'}`}
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <select
                         ...
                         hover:border-slate-600/50 transition-colors input-control pl-10 appearance-none ${errors.category ? 'border-red' : 'border-slate-700'}`}
                        >
                            {BILL_CATEGORIES.map(category => (
                                <option key={category.value} value={category.value} className='bg-slate-800 text-slate-200 py-2'>
                                    {category.label}
                                </option>
                            ))}
                        </select>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            className={`input-control pl-10 ${errors.amount ? 'border-red' : 'border-slate-700'}`}
                            placeholder="0.00"
                            min="0"
                            step="0.01"
                        />
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            value={formData.date}
                            onChange={handleChange}
                            className={`input-control pl-10 ${errors.date ? 'border-red' : 'border-slate-700'}`}
                        />
                    </div>
                    {errors.date && (
                        <p className="mt-2 text-sm text-red flex items-center gap-1">
